...
### Task 6: 录制的页面摘录不再带走输入框与编辑区里的内容（计划外，终审前加入）

**问题。** 录制每记一条页面条目，都会存一段最多 2000 字的页面正文摘录。摘录由 `electron-page-adapter.ts` 的 `readText` 产出：它把整棵可访问性树里所有 `StaticText` 节点的文字拼起来，而浏览器把输入框的值也作为 `StaticText` 挂在输入框节点下面。于是在真实 Chromium 里，人打过的一次性验证码会原样进摘录，密码以圆点形式暴露长度，编辑区里打的字也在其中。触发时机：开始录制时、每一次同文档的地址变化（`pushState`、锚点变化、`replaceState`）之后、以及后退回到那一页时（浏览器会恢复验证码框的值）；换到新文档或刷新之后不会。摘录进 `events.jsonl` 的页面事件，也进轨迹的 JSON 块，而提炼时整份轨迹都会放进发给 Agent 的提示里。这违反了 0.1.4 起「密码与一次性验证码从不被录下」的承诺。不属于六条待办，是做第三条时发现的。

**Files:**

- Modify: `src/main/browser/electron-page-adapter.ts`、`src/main/browser/types.ts`、`src/main/recording/session.ts`
- Test: `tests/recording-session.test.ts`、`tests/electron.e2e.ts`，以及适配器层能单测的部分
- Docs: `docs/architecture.md`（第三条写明了这条摘录通道仍会泄漏，修好后改掉）、`CHANGELOG.md` 的 `[未发布]`

**设计决定：**

1. **`readText` 本身不动。** 它同时服务于 Agent 的 `browser.snapshot` 与 `browser.page_info` 工具，Agent 读页面时看到什么不在这个任务的范围里。录制改用一个新的、专门的读取方法。
2. **新方法跳过一切可编辑内容**：属于输入框、文本域、可编辑组合框、数字框，以及富文本编辑区的文字，一律不进摘录，无论它是不是密码。理由：非密码输入框的值本来就由输入步骤记录，摘录再存一遍只增加风险；富文本编辑区与第三条、以及「富文本只报字符数」的规则保持一致。代价是页面上已有的编辑区内容（例如草稿）不再出现在提炼时的页面上下文里。
3. **判定用可访问性树自己的信息**，不另做 DOM 查询：可访问性节点带 `editable` 属性，并有父子关系；某个 `StaticText` 只要有一个祖先是可编辑节点或文本输入类角色，就跳过。具体用哪些属性与角色，由实现者在真实 Chromium 里确认，并在报告里写明依据。
4. 会话层的页面记录改用新方法；方法不存在时（测试用的假页面）照旧回退为空串。

**验收用例**（每条做变异验证）：

- **E2E，这是唯一有效的验收**：在测试页面上放一个一次性验证码框、一个密码框、一个富文本编辑区和一段普通文字；录制中依次输入一串独特的验证码、一个密码、一段独特的正文，然后触发一次同文档地址变化（`pushState` 或锚点变化），停止录制。读 `events.jsonl` 与 `trajectory.md` 全文：验证码、与密码等长的圆点串、那段正文都一次也不出现，而那段普通文字出现在摘录里。**修复前这条必须是红的。**
- Agent 读页面的路径不变：`readText` 的输出与修改前相同，可以用源码层面的断言或一次对照来证明。
- 单测：会话层在页面记录时调用的是新方法，而不是 `readText`。

- [ ] 写失败用例并确认红；实现；全量检查加录制相关 E2E；变异验证；改架构文档与更新日志；提交 `fix(recording): keep form and editor contents out of the page excerpt`
