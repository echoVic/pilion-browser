...
> 2026-09-24 录制待办终审修订：带标记、干净名不空的元素，页面还按改动之前的路径算一遍名字（带着人打的字），里面没有连着的干净名时，`el` 另带 `split: true`，只有这一位出页面；这时 observe 那一行名字相等也扣下，接受的行与改动之前一样。个数没变的错位（同标题的两张卡片换了先后）因此也拦得住，只要打的字夹在干净名中间。已知没有盖住的形状再加一种：包着把验证码每一位画成普通文字的组件、从内容取名的外壳，画出来的每一位是普通文字，照样读进干净名。
