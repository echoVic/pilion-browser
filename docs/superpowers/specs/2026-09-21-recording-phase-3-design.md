...
> 2026-09-23 录制待办 Task 3 修订：名字可能取自可编辑文字的元素（自己或祖先是编辑宿主、后代里有编辑宿主，或 `aria-labelledby` / `label` 指向这样的元素），脚本跳过编辑宿主的子树算名字，身处编辑宿主里的元素不从内容取名，并在 `el` 上标 `editable: true`；其余元素的 `el` 与以前逐字节相同。对带标记的元素，`target` 的解析多两道关：干净名是空串时，observe 那一行的名字也得是空串才算对上（空串被任何名字包含，序号错位时会把别的同类元素当成它）；对上之后，只有名字与脚本的干净名规范化后相等才照用那一行，否则扣下名字——`name` 为空串、带 `editable: true`、保留那一行的指纹、不带 `nth`；对不上、退回 `el` 时同样扣下名字，此时没有指纹。这样的目标回放只认指纹，对不上就交还，不按名字匹配。
