...
> 2026-09-23 录制待办 Task 6 修订：`page` 的 `text` 不再取自 Agent 读页面用的 `readText()`，改取页面适配器专门给录制的 `readRecordableText()`：取的节点与顺序照旧（没被忽略的 `StaticText` 与标题），但一段字自己或某个祖先带 `editable` 属性、或是 `textbox` / `searchbox` / `spinbutton` 角色就不要，子树里有这类节点、或名字取自 `aria-labelledby` 的标题整条不要。输入框、文本域、数字框、可编辑组合框与富文本编辑区里的字因此不进摘要，不论是不是密码与验证码；页面上已有的编辑区内容（比如草稿）也不再出现在提炼时的页面上下文里。适配器没有这个方法时摘要为空，不退回 `readText()`。

> 2026-09-24 录制待办终审修订：页面自己把人打的字另写成普通文字时（input-otp 把验证码每一位画进方框、上面盖着透明的验证码框；实时预览；「……的搜索结果」），树上看不出那是人打的，所以再拿跳过的那些 `StaticText`（框里的值与编辑区里的字）去比留下的行，标题那一行也比，比时不分大小写、空白折成一个：整行等于某个至少两个字的值、含有某个至少四个字的值，或者连着几行都只有一个字、拼起来是某个值（那几行），也不要。留下的字凑够 60000 个就不再往后比。往上找可编辑祖先时每个节点的结论随走随记，整棵树只判一遍；父子关系绕成环时，环上的字按可编辑算。
...
> 2026-09-24 录制待办终审修订：带标记、干净名不空的元素，页面还按改动之前的路径算一遍名字（带着人打的字），里面没有连着的干净名时，`el` 另带 `split: true`，只有这一位出页面；这时 observe 那一行名字相等也扣下；没有 `split` 时改动之前的名字包含那一行的名字，所以带标记的元素用上的行，改动之前一定也会用。个数没变的错位（同标题的两张卡片换了先后）因此也拦得住，只要打的字夹在干净名中间。已知没有盖住的形状再加一种：包着把验证码每一位画成普通文字的组件、从内容取名的外壳，画出来的每一位是普通文字，照样读进干净名。
...
- 密码与一次性验证码的过滤在页面脚本源头，日志里 `secret` 不带值也不带长度；`edit` 只记字符数不记内容。日志不是键盘记录器：`input` 记的是字段当时的值，与现在一致。
  > 2026-09-24 录制待办终审修订：密级字段是 `type="password"` 的输入框、`autocomplete` 标着 `current-password` 或 `new-password` 的输入框（不论 `type`），以及标着 `one-time-code` 的。脚本见过是密级的输入框，这份文档里一直算（「显示密码」把 `type` 换成 `text` 之后照样只报 `secret`）；脚本开始时、每次可信的 pointerdown 与 keydown 时，都先把页面上的密码框过一遍。脚本从没见过是密码框的明文框认不出来。
