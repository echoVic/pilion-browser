...
人可以录制自己在当前标签上的操作，得到一份行为轨迹；轨迹是 `recordings/<slug>/trajectory.md` 里的一个 ```json pilion-trajectory 代码块，上方的时间线由它渲染、加载时忽略，这个代码块的 `entries` 是从同目录 `events.jsonl` 算出来的投影，不是独立写入的另一份真相。步骤只有 `navigate / click / type / select / check / press / human / note` 八种，目标用角色、可访问名、标签、输入类型、同名序号与指纹前缀描述，不含任何只有 Pilion 认得的句柄；`ElementRef` 不落盘，因为它的三层身份（标签、文档 epoch、CDP nodeId）都是一次性的。轨迹文件是 `0o600` 的明文，里面没有密码也没有一次性验证码（那两类字段只留「需要我」步骤），但确实有你键入的文本（邮箱、搜索词等）、页面标题和正文摘录，所以它和浏览记录一样敏感。`events.jsonl` 是这份投影算出来的依据：只增不改的原始事件流，停止录制时一次性写入，权限同样是 `0o600`，同样没有密码和一次性验证码（页面脚本源头就过滤掉，不带值也不带长度），但同样有键入的文本、页面标题与正文摘录，敏感度与轨迹文件相同，比它更细——记的是每一次改动而不只是最终值。富文本编辑本身只记字符数、不记内容，编辑区里的正文也不经元素名落盘。一个元素自己或祖先是编辑宿主（`contenteditable` 且不为 `false`，所以编辑区里的链接、`contenteditable="false"` 的提及标签都算）、后代里有编辑宿主（包着编辑区的外壳），或者 `aria-labelledby`、`label` 指向这样的元素时，页面脚本算名字会跳过编辑宿主的整棵子树，身处编辑宿主里的元素完全不从内容取名，并在元素描述上标 `editable`；不在任何编辑上下文里的元素照旧直接读 `textContent`，载荷与以前逐字节相同。observe 的名字来自浏览器的可访问性树，按钮、链接、单元格这类角色取的是内部文字，同样会带出正文，所以采集层只在 observe 那一行的名字与脚本的干净名规范化后相等时才用它（带标签的编辑框就是这样，回放照常按名字匹配）；否则、以及对不上 observe 退回脚本描述时，一律扣下名字：目标的 `name` 为空串、带 `editable: true`、不带 `nth`，对上了 observe 那一行就保留它的指纹，时间线、步骤视图与审批摘要里写作「[名称已隐去，含富文本]」。扣下名字的目标回放只认指纹，指纹对不上就交还给人，绝不按名字匹配，因为空名或残缺的名字可能正好等于页面上另一个元素的名字。这是回放能力上的取舍：包着编辑区的元素，以前要编辑区里的字与录制时完全相同才能按名字匹配上，现在要外层 HTML 完全相同才能按指纹匹配上，两者大体相当；对不上时交还给人，不会点错。有一种情况连指纹也不留：干净名是空串（编辑区里的链接、只包着编辑区的外壳），observe 那一行的名字却不空。observe 是换页时预取的，之后插进页面的元素会让序号错位，这时分不清那一行是它本人还是错位后的别的同类元素，留下别人的指纹，回放就会点到别人身上；所以按对不上处理，扣下名字且不带指纹，回放到这一步直接交还。保留的指纹是外层 HTML 的哈希前 8 位，不是正文本身，但由正文算出，极短、可猜的内容理论上能被逐个试出来。还有一条口子不归元素名管：每次换页、包括页内地址变化时，页面条目会记下页面最前面 2000 字的正文摘录，编辑区里当时的字也在其中，所以你打完字之后页面改了地址，那段字仍会随摘录落盘。
...
`pnpm test` 覆盖策略、原子存储、帧校验、MCP、SSH 引号转义与真实 Unix socket MCP 回程。`pnpm test:e2e` 在隔离 profile 启动真实 Electron，验证浏览、主进程 IPC、审批、正文、对话、取消与恢复。`pnpm test` 另覆盖事件日志 schema 与 `v1`/`v2` 轨迹格式往返、目标匹配五级降级、录制脚本的 isTrusted、密码过滤、滚动节流与 iframe 收口、采集层的上限与目标解析、编辑上下文里的元素取名跳过编辑区且普通元素载荷逐字节不变、带正文的名字被扣下、干净名为空串时不认序号上名字不空的那一行、扣下名字的目标只按指纹回放、投影层的纯函数性质（同一份日志算两次结果相同）、日志与轨迹哈希不符时的重算、渲染给 Agent 的过程时间线折叠规则、录制会话与提炼生命周期编排、录制通道的命令顺序与回放状态机；`pnpm test:e2e` 覆盖真实录制一次点击、保存、不连 Agent 回放到目标页、目标消失时回放停在正确的步骤，以及录制中的滚动、后退与富文本输入进了事件日志——后退在步骤视图里是一条 navigate，技能库「过程」视图能看到折叠后的滚动次数与停顿；另有一条在带角色属性的外壳里的编辑区打字、页内地址变化让主进程重新观察之后再点一下，读 `events.jsonl` 与 `trajectory.md` 全文，确认打的字没有经页面脚本或可访问性树的元素名出现一次。测试 Agent 是确定性 ACP fixture，不代表生产模型质量或真实远端主机已经认证成功。
