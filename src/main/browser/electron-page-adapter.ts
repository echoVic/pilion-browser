...
import type {
  BrowserEffect, BrowserPageFactory, BrowserPagePort, PageLifecycleEvent,
  PageObservedElement, PageSnapshot,
} from './types.js';
...
type CdpNode = {
  nodeId: number; nodeName: string; nodeValue?: string; parentId?: number;
  attributes?: string[]; children?: CdpNode[];
};
type CdpResult<T> = T;
...
export class ElectronPagePort implements BrowserPagePort {
  ...
  constructor(
    ...
    wc.on('did-frame-navigate', (_event, _url, _httpCode, _httpStatus, isMainFrame) => {
      this.listener(isMainFrame ? { kind: 'document-committed' } : { kind: 'frame-committed', frameId: 'subframe' });
    });
    wc.on('destroyed', () => { this.closed = true; this.listener({ kind: 'destroyed' }); });
  ...
  async navigate(canonicalUrl: string, signal?: AbortSignal): Promise<void> {
    ...
    try { await this.view.webContents.loadURL(validated); throwIfAborted(signal); }
    finally {
  ...
  async observeElements(signal?: AbortSignal): Promise<ReadonlyArray<PageObservedElement>> {
    throwIfAborted(signal);
    const root = await this.command<{ root: CdpNode }>('DOM.getDocument', { depth: 0, pierce: false }, signal);
    const found = await this.command<{ nodeIds: number[] }>('DOM.querySelectorAll', { nodeId: root.root.nodeId, selector: SELECTOR }, signal);
    const rows: PageObservedElement[] = [];
    for (const nodeId of found.nodeIds.slice(0, 200)) {
      const described = await this.command<{ node: CdpNode }>('DOM.describeNode', { nodeId, depth: 3 }, signal);
      const attrs = attributes(described.node.attributes ?? []);
      const outer = await this.command<{ outerHTML: string }>('DOM.getOuterHTML', { nodeId }, signal);
      ...
      rows.push({
        elementKey: String(nodeId), frameId: 'main', localFingerprint: elementFingerprint(outer.outerHTML, semantic.checked),
        role: semantic.role, name: semantic.name, disabled: Object.hasOwn(attrs, 'disabled') || attrs['aria-disabled'] === 'true',
        tagName, inputType: semantic.inputType, formAction: semantic.formAction,
        optionValues, checked: semantic.checked,
      });
    }
    return rows;
  }

  async elementFingerprint(frameId: string, elementKey: string, signal?: AbortSignal): Promise<string | undefined> {
    throwIfAborted(signal);
    if (frameId !== 'main') return undefined;
    try {
      const id = nodeId(elementKey);
      const result = await this.command<{ outerHTML: string }>('DOM.getOuterHTML', { nodeId: id }, signal);
      const described = await this.command<{ node: CdpNode }>('DOM.describeNode', { nodeId: id, depth: 0 }, signal);
      const semantic = await this.semanticDetails(described.node, attributes(described.node.attributes ?? []), signal);
      return elementFingerprint(result.outerHTML, semantic.checked);
    } catch { return undefined; }
  }

  async applyEffect(frameId: string, elementKey: string, effect: BrowserEffect, signal?: AbortSignal): Promise<void> {
    throwIfAborted(signal);
    if (frameId !== 'main') throw new BrowserError('UNSUPPORTED_ELEMENT', 'Only observed main-frame elements are supported');
    const id = nodeId(elementKey);
    const described = await this.command<{ node: CdpNode }>('DOM.describeNode', { nodeId: id, depth: 3 }, signal);
    const attrs = attributes(described.node.attributes ?? []);
    const tagName = described.node.nodeName.toLowerCase();
    ...
    if (effect.kind === 'type') {
      ...
      if (effect.replace !== false) {
        await this.command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'a', code: 'KeyA', modifiers: 2 }, signal);
        await this.command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'a', code: 'KeyA', modifiers: 2 }, signal);
      }
      await this.command('Input.insertText', { text: effect.text }, signal);
      return;
    }
    if (effect.kind === 'press') {
      if (!PRESS_KEYS.includes(effect.key) || !Array.isArray(effect.modifiers) ||
    ...
    if (effect.kind === 'select') {
      if (tagName !== 'select') throw new BrowserError('UNSUPPORTED_ELEMENT', 'browser.select requires a <select> target');
      const options = selectOptionValues(described.node);
      const optionIndex = options.indexOf(effect.value);
      if (optionIndex < 0) throw new BrowserError('OPTION_NOT_FOUND', 'Requested option does not exist in the current <select>');
      await this.command('DOM.focus', { nodeId: id }, signal);
      await this.dispatchKey('Home', 0, signal);
      for (let index = 0; index < optionIndex; index += 1) await this.dispatchKey('ArrowDown', 0, signal);
      await this.dispatchKey('Enter', 0, signal);
      return;
    }
    if (effect.kind === 'check') {
      const inputType = tagName === 'input' ? (attrs.type || 'text').toLowerCase() : '';
      if (!['checkbox', 'radio'].includes(inputType)) throw new BrowserError('UNSUPPORTED_ELEMENT', 'browser.check requires a checkbox or radio');
      if (inputType === 'radio' && !effect.checked) throw new BrowserError('UNSUPPORTED_ELEMENT', 'A radio control cannot be unchecked directly');
      const semantic = await this.semanticDetails(described.node, attrs, signal);
      if (semantic.checked === effect.checked) return;
      if (semantic.checked === undefined) throw new BrowserError('UNSUPPORTED_ELEMENT', 'Current checked state is unavailable');
    }
    await this.clickNode(id, signal);
  }

  setLifecycleListener(listener: (event: PageLifecycleEvent) => void): void { this.listener = listener; }

  close(): void {
    if (this.closed) return;
    this.closed = true;
    if (this.view.webContents.debugger.isAttached()) this.view.webContents.debugger.detach();
    try { this.parent.contentView.removeChildView(this.view); } catch { /* parent may already be closing */ }
  ...
  private guardNavigation(url: string): boolean {
    ...
    void this.validateUrl(url).then(canonical => this.navigate(canonical)).catch(() => {
      // Navigation remains cancelled: URL policy failures are fail-closed.
    });
    return false;
  }

  private async semanticDetails(node: CdpNode, attrs: Record<string, string>, signal?: AbortSignal): Promise<{ role: string; name: string; inputType?: string; formAction?: string; checked?: boolean }> {
    ...
    try {
      const ax = await this.command<{ nodes?: Array<{
      ...
      const checkedValue = ax.nodes?.[0]?.properties?.find(property => property.name === 'checked')?.value?.value;
      if (checkedValue === true || checkedValue === 'true') checked = true;
      else if (checkedValue === false || checkedValue === 'false') checked = false;
    } catch { /* AX may be unavailable during navigation. */ }
    const inputType = node.nodeName.toLowerCase() === 'input' ? (attrs.type || 'text').toLowerCase() : undefined;
    const rawAction = attrs.formaction || await this.findAncestorAttribute(node.parentId, 'form', 'action', signal);
    let formAction: string | undefined;
    if (rawAction) {
      try { formAction = new URL(rawAction, this.view.webContents.getURL()).toString(); }
      catch { formAction = rawAction.slice(0, 500); }
    }
    return { role, name: name.trim().slice(0, 200), inputType, formAction, checked };
  }

  private async findAncestorAttribute(parentId: number | undefined, tag: string, attribute: string, signal?: AbortSignal): Promise<string | undefined> {
    let current = parentId;
    for (let depth = 0; current && depth < 32; depth += 1) {
      const row = await this.command<{ node: CdpNode }>('DOM.describeNode', { nodeId: current, depth: 0 }, signal);
  ...
  private async clickNode(id: number, signal?: AbortSignal): Promise<void> {
    const box = await this.command<{ model: { content: number[] } }>('DOM.getBoxModel', { nodeId: id }, signal);
  ...
  private async dispatchKey(key: typeof PRESS_KEYS[number], modifiers: number, signal?: AbortSignal): Promise<void> {
    const protocol = keyDetails(key);
    await this.command('Input.dispatchKeyEvent', { type: 'keyDown', ...protocol, modifiers }, signal);
    await this.command('Input.dispatchKeyEvent', { type: 'keyUp', ...protocol, modifiers }, signal);
  }

  private async command<T = unknown>(method: string, params?: Record<string, unknown>, signal?: AbortSignal): Promise<CdpResult<T>> {
    throwIfAborted(signal);
...
export class ElectronPageFactory implements BrowserPageFactory {
  ...
  async create(canonicalUrl: string): Promise<BrowserPagePort> {
    const view = new WebContentsView({ webPreferences: {
      ...
      webSecurity: true, partition: 'persist:pilion-default',
    } });
...
export function hardenUntrustedContents(contents: WebContents, allowNavigation: (url: string) => boolean): void {
...
function elementFingerprint(outerHtml: string, checked: boolean | undefined): string {
  return createHash('sha256').update(`${outerHtml}\nchecked:${checked === undefined ? 'na' : String(checked)}`).digest('hex');
}
function throwIfAborted(signal?: AbortSignal): void {
  if (signal?.aborted) throw signal.reason instanceof Error ? signal.reason : new Error('Browser operation revoked');
}
...
function selectOptionValues(select: CdpNode): string[] {
  ...
  const visit = (node: CdpNode, ancestorDisabled = false): void => {
    ...
    if (tagName === 'option') {
      if (disabled) return;
      const text = (node.children ?? []).map(child => child.nodeValue ?? '').join('').trim();
...
function keyDetails(key: typeof PRESS_KEYS[number]): { key: string; code: string } {
  ...
  const codes: Record<typeof PRESS_KEYS[number], string> = {
    ...
    PageUp: 'PageUp', PageDown: 'PageDown', Backspace: 'Backspace', Delete: 'Delete', Space: 'Space',
  };
