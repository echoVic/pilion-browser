import { createServer, request as httpRequest, type ClientRequest, type IncomingMessage, type OutgoingHttpHeaders, type RequestOptions, type Server as HttpServer } from 'node:http';
...
type Dial = (options: { host: string; port: number; family: 4 | 6 }, onConnect?: () => void) => Socket;
...
export class ControlledNetworkProxy {
  ...
  constructor(private readonly options: ControlledProxyOptions) {
    ...
    this.server = createServer((request, response) => { void this.forwardHttp(request, response); });
    this.server.on('connect', (request, client, head) => { void this.forwardConnect(request, client, head); });
    this.server.on('upgrade', (request, client, head) => { void this.forwardUpgrade(request, client, head); });
  ...
  async listen(): Promise<{ host: '127.0.0.1'; port: number }> {
    ...
    if (!address || typeof address === 'string') throw new Error('Controlled proxy failed to bind TCP');
  ...
  private async forwardHttp(incoming: IncomingMessage, response: import('node:http').ServerResponse): Promise<void> {
    try {
      const url = parseAbsoluteUrl(incoming.url, 'http:');
      if (url.protocol !== 'http:') throw new BrowserError('DANGEROUS_URL', 'HTTPS must use CONNECT');
      const target = await resolvePinnedTarget(url.hostname, portOf(url, 80), this.options.resolver);
      ...
      const outbound = this.requestFactory({
        host: target.address, family: target.family, port: target.port,
        method: incoming.method, path: `${url.pathname}${url.search}`, headers,
  ...
  private async forwardConnect(request: IncomingMessage, client: Duplex, head: Buffer): Promise<void> {
    try {
      ...
      const target = await resolvePinnedTarget(authority.hostname, authority.port, this.options.resolver);
      if (client.destroyed) return;
      const upstream = this.track(this.dial({ host: target.address, port: target.port, family: target.family }));
  ...
  private async forwardUpgrade(request: IncomingMessage, client: Duplex, head: Buffer): Promise<void> {
    try {
      ...
      if (url.protocol !== 'ws:') throw new BrowserError('DANGEROUS_URL', 'Secure WebSocket must use CONNECT');
      const target = await resolvePinnedTarget(url.hostname, portOf(url, 80), this.options.resolver);
      if (client.destroyed) return;
      const upstream = this.track(this.dial({ host: target.address, port: target.port, family: target.family }));
      upstream.once('connect', () => {
        if (client.destroyed) { upstream.destroy(); return; }
        const headers = Object.entries({ ...request.headers, host: url.host })
          .flatMap(([name, value]) => value === undefined ? [] : [`${name}: ${Array.isArray(value) ? value.join(', ') : value}`]);
        upstream.write(`${request.method ?? 'GET'} ${url.pathname}${url.search} HTTP/${request.httpVersion}\r\n${headers.join('\r\n')}\r\n\r\n`);
        if (head.length) upstream.write(head);
        upstream.pipe(client); client.pipe(upstream);
      });
      tunnelFailClosed(client, upstream);
...
export async function resolvePinnedTarget(hostname: string, port: number, resolver: HostResolver): Promise<PinnedTarget> {
  const normalized = hostname.replace(/^\[|\]$/g, '').toLowerCase();
  if (!normalized || !Number.isInteger(port) || port < 1 || port > 65_535) throw new BrowserError('INVALID_ARGUMENT', 'Invalid proxy target');
  const addresses = await resolver.resolve(normalized);
  if (!addresses.length || addresses.some(address => isIP(address) === 0 || isPrivateAddress(address))) {
    throw new BrowserError('PRIVATE_NETWORK_BLOCKED', 'Proxy target resolved to a private or invalid address');
  }
...
function parseAbsoluteUrl(raw: string | undefined, expected: 'http:' | 'ws:'): URL {
  ...
  if (url.protocol !== expected || url.username || url.password || !url.hostname) throw new BrowserError('DANGEROUS_URL', 'Proxy request target is not allowed');
  return url;
}
function parseAuthority(raw: string | undefined, fallbackPort: number): { hostname: string; port: number } {
...
function portOf(url: URL, fallback: number): number { return url.port ? Number(url.port) : fallback; }
function failHttp(response: import('node:http').ServerResponse): void {
  if (!response.headersSent) response.writeHead(502, { connection: 'close', 'content-type': 'text/plain' });
