...
function cborHead(major: number, value: number): Buffer {
  ...
  const result = Buffer.alloc(9); result[0] = (major << 5) | 27; result.writeBigUInt64BE(BigInt(value), 1); return result;
}
...
export function canonicalCborEncode(value: unknown): Buffer {
  ...
  if (typeof value === "number") {
    ...
    if (Number.isSafeInteger(number)) return cborHead(number >= 0 ? 0 : 1, number >= 0 ? number : -1 - number);
    const result = Buffer.alloc(9); result[0] = 0xfb; result.writeDoubleBE(number, 1); return result;
  }
