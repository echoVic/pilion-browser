...
export type UnsignedExecutionGrant = Omit<ExecutionGrant, "schemaVersion" | "grantId" | "issuedAt" | "expiresAt" | "signature"> & {
  schemaVersion?: "1";
  grantId?: string;
  issuedAt?: string;
  expiresAt?: string;
...
const grantKeys = [
  "schemaVersion", "grantId", "hostInstanceId", "profileId", "sessionId", "attachmentId",
  "connectionEpoch", "tabId", "actionId", "attemptId", "canonicalCommandHash", "actionDigest",
  "capabilitySnapshotHash", "policySetVersion", "policyVerdict", "approvalId", "approvalDigest",
  "fencingToken", "issuedAt", "deadline", "expiresAt", "obligations", "signature",
] as const;
...
function validDate(value: string): number {
  ...
  if (!Number.isFinite(result)) throw new HostError("GRANT_INVALID", "Grant contains an invalid timestamp");
  return result;
}
...
export class ExecutionGrantAuthority {
  ...
  issue(input: UnsignedExecutionGrant, now = new Date()): ExecutionGrant {
    ...
    if (unsigned.policyVerdict === "require_approval" && (!unsigned.approvalId || !unsigned.approvalDigest)) {
  ...
  verifyAndConsume(
    ...
    if (!grant || typeof grant !== "object" || Object.keys(grant).some((key) => !grantKeys.includes(key as typeof grantKeys[number]))) {
      throw new HostError("GRANT_INVALID", "Grant schema has unknown fields");
    }
    if (grant.schemaVersion !== "1" || !secureEqual(grant.signature, mac(this.key, unsignedGrant(grant)))) {
      throw new HostError("GRANT_INVALID", "Grant signature is invalid");
    }
    const nowMs = now.getTime();
    if (validDate(grant.expiresAt) <= nowMs || validDate(grant.deadline) <= nowMs || validDate(grant.issuedAt) > nowMs + 5_000) {
    ...
    if (expected.minFencingTokenExclusive !== undefined && grant.fencingToken <= expected.minFencingTokenExclusive) {
    ...
    for (const obligation of grant.obligations) {
      ...
      if (!allowed || Object.keys(obligation.parameters).some((field) => !allowed.includes(field))) {
