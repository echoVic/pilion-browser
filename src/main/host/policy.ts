...
export function classifySemanticRisk(context: SemanticContext): RiskClassification {
  ...
  if (context.destination === "file-upload" || context.operation === "upload") reasons.push("FILE_EXFILTRATION");
  if (context.operation === "download-executable") reasons.push("EXECUTABLE_DOWNLOAD");
  if (context.targetOrigin && !/^https?:\/\//i.test(context.targetOrigin)) reasons.push("EXTERNAL_PROTOCOL");
  if (
  ...
  const highRisk = Boolean(context.changesExternalState) || reasons.some((reason) => !["UNCLASSIFIED", "CONTEXT_MISSING"].includes(reason));
  let effect: EffectLevel;
  if (highRisk || context.changesExternalState) effect = "external-side-effect";
  else if (/observe|read|list|page_info|screenshot/i.test(context.operation ?? "")) effect = "pure-observe";
  ...
  if (reasons.length === 0) reasons.push(
    effect === "pure-observe" ? "PURE_OBSERVATION" : effect === "viewport-mutating" ? "VIEWPORT_CHANGE" : "PAGE_STATE_CHANGE",
  );
  return { effect, highRisk, certain, reasons: [...new Set(reasons)] };
}

export type Obligation = { type: "revalidate_target" | "trusted_approval" | "https_only"; parameters: Record<string, unknown> };
...
export function evaluatePolicy(input: {
  ...
  if (!input.policyLoaded) return { verdict: "deny", policySetVersion: version, reasonCodes: ["POLICY_UNAVAILABLE"], obligations: [] };
  if (!input.contextComplete || !input.classification) {
    return input.trustedApprovalAvailable
      ? { verdict: "require_approval", policySetVersion: version, reasonCodes: ["CONTEXT_MISSING"], obligations: [{ type: "trusted_approval", parameters: {} }, { type: "revalidate_target", parameters: {} }] }
      : { verdict: "deny", policySetVersion: version, reasonCodes: ["CONTEXT_MISSING"], obligations: [] };
  }
  const risk = input.classification;
  if ((!risk.certain || risk.highRisk) && !input.trustedApprovalAvailable) {
    return { verdict: "deny", policySetVersion: version, reasonCodes: risk.reasons, obligations: [] };
  }
  if (!risk.certain || risk.highRisk) {
    return { verdict: "require_approval", policySetVersion: version, reasonCodes: risk.reasons, obligations: [{ type: "trusted_approval", parameters: {} }, { type: "revalidate_target", parameters: {} }] };
  }
  return { verdict: "allow", policySetVersion: version, reasonCodes: risk.reasons, obligations: risk.effect === "pure-observe" ? [] : [{ type: "revalidate_target", parameters: {} }] };
}
