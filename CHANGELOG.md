...
- 打开一份被手改过的录制之前，如果先新建了另一份录制、或者改了另一份的名字，「步骤已按过程记录重算，手工改动未保留」这句提示此前会永远看不到——它只在详情面板里出现那一次，而这些操作都会刷新技能库列表，把提示顺带带走。现在这条提示会一直记着，直到你真的点开那份录制的详情看过一次；没看过的那一行在列表里会带一个「步骤已重算」的短标记，不用一份份点开也能找到是哪份
