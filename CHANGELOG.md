...
- 录制时点了包着富文本编辑器的元素（比如嵌着编辑区的卡片），或者点了编辑区里的链接、@提及，你在编辑器里打的字此前会被当成这个元素的名字，写进录制文件，还会出现在步骤列表和回放前请你确认的摘要里。现在这类元素的名字会隐去，显示为「[名称已隐去，含富文本]」。名字隐去的步骤，回放时不会凭名字或记错的位置点到别的元素上，而是停在这一步并告诉你是哪一步；由 Agent 发起的回放会把这一步交还给 Agent。所以录制里点进富文本编辑器的那几下，名字一旦隐去，回放到那里就会停下
