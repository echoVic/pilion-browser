...
- 录制时点了包着富文本编辑器的元素（比如嵌着编辑区的卡片），或者点了编辑区里的链接、@提及，你在编辑器里打的字此前会被当成这个元素的名字，写进录制文件，还会出现在步骤列表和回放前请你确认的摘要里。现在这类元素的名字会隐去，显示为「[名称已隐去，含富文本]」；回放时只按元素本身的样子去找它，找不到就停下来交给你，不会凭名字点到别的元素上
