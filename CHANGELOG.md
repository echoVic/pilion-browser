...
- 录制时点了包着富文本编辑器的元素（比如嵌着编辑区的卡片），或者点了编辑区里的链接、@提及，你在编辑器里打的字此前会被当成这个元素的名字，写进录制文件，还会出现在步骤列表和回放前请你确认的摘要里。现在这类元素的名字会隐去，显示为「[名称已隐去，含富文本]」；回放到这一步会停下来交给你，不会凭名字或记错的位置点到别的元素上
- 录制时点了包着密码框或验证码框的元素（比如把输入框和说明文字包在一起的登录卡片），此前它的名字会带出验证码本身，或者一串个数正好等于密码长度的圆点，写进录制文件。现在这类元素的名字同样会隐去，密码和验证码的值与长度都不会再离开网页
