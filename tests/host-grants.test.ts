...
describe('ExecutionGrant', () => {
  it('验证 HMAC、epoch、expiry 并执行单次消费', () => {
    ...
    const store = {
      consumeGrant: (grant: Pick<ExecutionGrant, 'grantId'>) =>
        !consumed.has(grant.grantId) && (consumed.add(grant.grantId), true),
    };
    const grant = authority.issue(base, now);
    expect(authority.verifyAndConsume(grant, expected, store, now)).toBe(grant);
    expect(codeOf(() => authority.verifyAndConsume(grant, expected, store, now))).toBe(
    ...
    expect(
      codeOf(() =>
        authority.verifyAndConsume(fresh, { ...expected, connectionEpoch: 3 }, store, now),
      ),
    ).toBe('STALE_CONNECTION_EPOCH');
    expect(
      codeOf(() =>
        authority.verifyAndConsume(
          authority.issue({ ...base, grantId: 'expired' }, now),
          expected,
          store,
  ...
  it('拒绝篡改、未知 obligation 与缺少审批绑定', () => {
    ...
    const grant = authority.issue(base, now);
    expect(
      codeOf(() => authority.verifyAndConsume({ ...grant, tabId: 'evil' }, expected, store, now)),
    ).toBe('GRANT_INVALID');
    const unknown = authority.issue(
      { ...base, grantId: 'unknown', obligations: [{ type: 'future', parameters: {} }] },
      now,
    );
    expect(codeOf(() => authority.verifyAndConsume(unknown, expected, store, now))).toBe(
      'GRANT_INVALID',
    );
    expect(() => authority.issue({ ...base, policyVerdict: 'require_approval' }, now)).toThrowError(
      HostError,
    );
  });
  ...
  it('拒绝畸形或非规范 base64url 签名', () => {
    ...
    for (const signature of signatures) {
      ...
      expect(codeOf(() => authority.verifyAndConsume(candidate, expected, store, now))).toBe(
        'GRANT_INVALID',
      );
    }
...
describe('ResumeCredential', () => {
  it('绑定身份、Host、能力快照和 epoch，nonce 仅消费一次', () => {
    ...
    const store = {
      consumeResumeNonce: (nonce: string) => !seen.has(nonce) && (seen.add(nonce), true),
    };
    expect(authority.verifyAndConsume(credential, identity, store, now)).toBe(credential);
    expect(codeOf(() => authority.verifyAndConsume(credential, identity, store, now))).toBe(
      'STALE_CONNECTION_EPOCH',
    );
    const other = authority.issue(identity, now);
    expect(
      codeOf(() =>
        authority.verifyAndConsume(other, { ...identity, connectionEpoch: 5 }, store, now),
      ),
    ).toBe('STALE_CONNECTION_EPOCH');
    const tampered = { ...authority.issue(identity, now), capabilitySnapshotHash: 'other' };
    expect(codeOf(() => authority.verifyAndConsume(tampered, identity, store, now))).toBe(
      'GRANT_INVALID',
    );
  });
});
