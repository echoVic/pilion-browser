...
import { DurableHostStore, HostError, ResumeCredentialAuthority, type IntentInput } from "../src/main/host/index";
...
describe("SQLite durable store", () => {
  it("Intent 与 Outbox 同事务提交，失败时一起回滚", () => {
    ...
    expect(() => store.createIntent(intent({ actionId: "duplicate", idempotencyKey: "idempotency-key" }))).toThrow();
  ...
  it("审批、prepare、dispatched、effect_started、Result 各自形成事务屏障", () => {
    ...
    expect(() => store.resolveApproval({ approvalId: "approval", nonce: "nonce", actionDigest: "action-digest", decision: "approve", bindingValid: true })).toThrowError(HostError);
  ...
  it("拒绝旧 epoch，重启 reconcile 将未知副作用收敛为 outcome_unknown", () => {
    ...
    try {
      stale.createIntent(intent({ actionId: "another", idempotencyKey: "other", connectionEpoch: 1 }));
    } catch (error) {
    ...
    store.prepareExecution({
      actionId: "action", attemptId: "attempt", sessionId: "session", attachmentId: "attachment",
      connectionEpoch: 2, tabId: "tab", policySetVersion: "policy-v1", capabilitySnapshotHash: "caps",
    });
    store.markDispatched("attempt", "grant");
    store.markEffectStarted("attempt");
    expect(store.reconcileExecuting({ attempt: "unknown" })).toEqual([{ attemptId: "attempt", outcome: "outcome_unknown" }]);
  ...
  it("恢复凭证消费、epoch 递增和新 Attachment 原子提交", () => {
    ...
    const identity = {
      principal: "principal", agentId: "agent", hostInstanceId: "host",
      capabilitySnapshotHash: "caps", connectionEpoch: 2,
    };
    const credential = authority.issue(identity, clock);
    expect(authority.verifyAndResume(credential, identity, store, {
      attachmentId: "replacement", leaseExpiresAt: "2026-01-01T00:01:00.000Z",
    }, clock)).toBe(3);
    expect(() => authority.verifyAndResume(credential, identity, store, {
      attachmentId: "replay", leaseExpiresAt: "2026-01-01T00:01:00.000Z",
    }, clock)).toThrowError(HostError);
  ...
  it("未消费 Grant 的 prepared Attempt 可证明未分发", () => {
    ...
    expect(store.reconcileExecuting({ attempt: "not_consumed" })).toEqual([{ attemptId: "attempt", outcome: "failed_before_dispatch" }]);
  ...
  it("过期 Attachment 在 createIntent 前原子标记 expired", () => {
    const store = new DurableHostStore({ path: ":memory:", now: () => clock });
    stores.push(store);
    store.createSession({ sessionId: "session", profileId: "profile", principal: "principal", agentId: "agent", connectionEpoch: 2, capabilitySnapshotHash: "caps" });
    store.createAttachment({
      attachmentId: "attachment", sessionId: "session", principal: "principal", agentId: "agent", role: "owner",
      leaseExpiresAt: clock.toISOString(), connectionEpoch: 2, capabilitySnapshotHash: "caps",
    });
    expect(() => store.createIntent(intent())).toThrowError(expect.objectContaining({ code: "LEASE_EXPIRED" }));
    expect(store.database.prepare("SELECT state FROM attachments WHERE attachment_id = ?").get("attachment")).toMatchObject({ state: "expired" });
  ...
  it("Intent 后 lease 到期会阻断 prepareExecution 并持久化 expired", () => {
    ...
    store.createAttachment({
      attachmentId: "attachment", sessionId: "session", principal: "principal", agentId: "agent", role: "owner",
      leaseExpiresAt: "2026-01-01T00:00:01.000Z", connectionEpoch: 2, capabilitySnapshotHash: "caps",
    });
    store.createIntent(intent());
    now = new Date("2026-01-01T00:00:02.000Z");
    expect(() => store.prepareExecution({
      actionId: "action", attemptId: "attempt", sessionId: "session", attachmentId: "attachment",
      connectionEpoch: 2, tabId: "tab", policySetVersion: "policy-v1", capabilitySnapshotHash: "caps",
