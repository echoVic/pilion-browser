...
describe('SSH ACP connection', () => {
  it('quotes remote arguments without shell expansion and requires private forwarding', () => {
    ...
    expect(execFileSync('/bin/sh', ['-c', `printf %s ${shellQuote(literal)}`], { encoding: 'utf8' })).toBe(literal);
    const launch = sshLaunch({ id: 'a', name: 'Remote', command: '/opt/agent', args: [literal], cwd: '/home/work space', enabled: true, transport: 'ssh', ssh: { host: 'user@remote', port: 2222 }, env: { TEST_VALUE: literal } }, '/tmp/local.sock', '/tmp/remote.sock');
    expect(launch.command).toBe('ssh');
    expect(launch.args).toContain('BatchMode=yes'); expect(launch.args).toContain('ExitOnForwardFailure=yes');
    ...
    expect(() => sshLaunch({ id: 'a', name: 'Bad', command: 'a', args: [], enabled: true, ssh: { host: '-oProxyCommand=bad', port: 22 } }, '', '')).toThrow();
  });
  it('runs MCP over the remote return stream, validates tool inputs, and tears down connections', async () => {
    ...
    const calls: string[] = [];
    const host = new BrowserMcpHost({ directory, bridgePath: '', execute: async request => { calls.push(request.name); return { text: 'Remote browser content' }; } });
    let client: Client | undefined;
    try {
      const server = await host.start('/tmp/remote.sock');
      expect(server).toEqual({ name: 'pilion-browser', command: 'nc', args: ['-U', '/tmp/remote.sock'], env: [] });
      expect((await stat(host.socketPath)).mode & 0o777).toBe(0o600);
      client = new Client({ name: 'remote-agent', version: '1.0.0' });
      await client.connect(new StdioClientTransport({ command: 'nc', args: ['-U', host.socketPath] }));
      ...
      expect(result.content).toEqual([{ type: 'text', text: JSON.stringify({ text: 'Remote browser content' }) }]);
    ...
    } finally { await client?.close(); await host.stop(); await rm(directory, { recursive: true, force: true }); }
  });
});
