...
describe("Host Core 状态机与 canonical digest", () => {
  ...
  it("语义风险与 Policy fail-closed，不依赖 click 工具名放行", () => {
    ...
    expect(evaluatePolicy({ classification: payment, policySetVersion: "v1", policyLoaded: true, trustedApprovalAvailable: true, contextComplete: true }).verdict).toBe("require_approval");
    expect(evaluatePolicy({ classification: payment, policySetVersion: "v1", policyLoaded: true, trustedApprovalAvailable: false, contextComplete: true }).verdict).toBe("deny");
    expect(evaluatePolicy({ policyLoaded: false, trustedApprovalAvailable: true, contextComplete: false }).verdict).toBe("deny");
  });

  it("敏感输入、缺失语义和任意外部副作用均默认要求审批", () => {
    const password = classifySemanticRisk({
      operation: "type", elementRole: "textbox", accessibleName: "Password", inputType: "password",
      targetOrigin: "https://accounts.example", changesExternalState: true, classifierConfident: true,
    });
    expect(password.reasons).toContain("SENSITIVE_INPUT");
    expect(evaluatePolicy({ classification: password, policyLoaded: true, trustedApprovalAvailable: true, contextComplete: true }).verdict).toBe("require_approval");

    const missingSemantics = classifySemanticRisk({ operation: "click", targetOrigin: "https://example.com", classifierConfident: false });
    expect(missingSemantics.certain).toBe(false);
    expect(evaluatePolicy({ classification: missingSemantics, policyLoaded: true, trustedApprovalAvailable: true, contextComplete: false }).verdict).toBe("require_approval");

    const external = classifySemanticRisk({
      operation: "custom-action", elementRole: "button", accessibleName: "Continue",
      targetOrigin: "https://example.com", changesExternalState: true, classifierConfident: true,
    });
    expect(external.effect).toBe("external-side-effect");
    expect(evaluatePolicy({ classification: external, policyLoaded: true, trustedApprovalAvailable: true, contextComplete: true }).verdict).toBe("require_approval");
  });
});
