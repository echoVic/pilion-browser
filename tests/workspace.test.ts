...
describe('workspace persistence', () => {
  it('restores conversations, bookmarks and tabs, and marks interrupted messages cancelled', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'pilion-workspace-')); directories.push(directory);
    const path = join(directory, 'workspace.json');
    const store = new WorkspaceStore(path); await store.load();
    expect(store.data.permissionMode).toBe('full');
    store.data.permissionMode = 'ask';
    store.create('c1', 'agent1');
    store.append({ id: 'm1', role: 'user', text: 'Research this page', time: new Date().toISOString() });
    store.append({ id: 'm2', role: 'assistant', text: 'partial', time: new Date().toISOString(), status: 'running' });
    store.data.tabs = ['https://example.com/'];
    store.data.bookmarks = [{ url: 'https://example.com/', title: 'Example', time: new Date().toISOString() }];
    await store.save();
    const restored = new WorkspaceStore(path); await restored.load();
  ...
  it('serializes writes so older snapshots cannot replace newer state', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'pilion-workspace-')); directories.push(directory);
    const path = join(directory, 'workspace.json'); const store = new WorkspaceStore(path);
    store.create('c1');
    const first = store.save(); store.create('c2'); const second = store.save();
  ...
  it('deduplicates visits and excludes internal pages', () => {
    ...
    for (const title of ['old', 'new']) store.visit({ title, url: 'https://example.com', time: 'now' });
  ...
  it('distinguishes addresses from search text and retains explicit protocols for policy validation', () => {
    ...
    expect(addressToUrl('research a topic')).toBe('https://www.google.com/search?q=research%20a%20topic');
