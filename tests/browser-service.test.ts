...
class FakePage implements BrowserPagePort {
  ...
  async snapshot() { return { url: 'https://example.com/', title: 'Example', loading: false }; }
  async navigate(url: string) { this.navigations.push(url); }
  async observeElements() {
    return [{
      elementKey: 'internal-node-7', frameId: 'main', localFingerprint: this.fingerprint,
      role: this.role, name: 'Save', disabled: false, tagName: this.tagName,
      inputType: this.inputType, optionValues: this.optionValues, checked: this.checked,
    }];
  }
  async elementFingerprint() { return this.fingerprint || undefined; }
  async applyEffect(_frameId: string, elementKey: string, effect: BrowserEffect) {
    expect(elementKey).toBe('internal-node-7');
    this.effects.push(effect);
  }
  setLifecycleListener(listener: (event: PageLifecycleEvent) => void) { this.listener = listener; }
  close() { return undefined; }
  emit(event: PageLifecycleEvent) { this.listener(event); }
}
...
describe('BrowserService element lifetime', () => {
  it('returns trusted observed semantics and rejects a forged ref', async () => {
    ...
    await expect(f.service.describeElement('owner', f.tabId, f.ref)).resolves.toEqual({ role: 'button', name: 'Save', disabled: false, tagName: 'button' });
    await expect(f.service.describeElement('owner', f.tabId, { ...f.ref, localFingerprint: 'forged' }))
  ...
  it('rejects a ref after a document commit', async () => {
    ...
    await expect(f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref,
      effect: { kind: 'click' }, grant: 'signed-grant',
    })).rejects.toSatisfy(expectCode('STALE_ELEMENT'));
  ...
  it('rejects forged ref fields even when the opaque id exists', async () => {
    ...
    await expect(f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: forged,
      effect: { kind: 'click' }, grant: 'signed-grant',
    })).rejects.toSatisfy(expectCode('STALE_ELEMENT'));
  });
});
...
describe('Tab ACL', () => {
  it('enforces observer/operator/owner permissions', async () => {
    const f = await observed();
    f.service.setTabAcl('owner', f.tabId, { entries: [
      { principalId: 'owner', role: 'owner' },
      { principalId: 'reader', role: 'observer' },
      { principalId: 'agent', role: 'operator' },
    ] });

    await expect(f.service.observe({ principalId: 'reader', tabId: f.tabId })).resolves.toBeDefined();
    await expect(f.service.navigate({ principalId: 'reader', tabId: f.tabId, url: 'example.org' }))
      .rejects.toSatisfy(expectCode('PERMISSION_DENIED'));
    await expect(f.service.navigate({ principalId: 'agent', tabId: f.tabId, url: 'example.org' }))
      .resolves.toEqual({ url: 'https://example.org/' });
    expect(() => f.service.setTabAcl('agent', f.tabId, { entries: [{ principalId: 'agent', role: 'owner' }] }))
      .toThrowError(BrowserError);
  });
});

describe('URL policy', () => {
  it.each(['javascript:alert(1)', 'file:///etc/passwd', 'data:text/html,x'])('blocks dangerous protocol %s', async url => {
  ...
  it('installs a fail-closed request hook for documents, subresources and WebSockets', async () => {
    ...
    let listener: ((details: { url: string }, callback: (result: { cancel: boolean }) => void) => void) | undefined;
    const fakeSession = { webRequest: { onBeforeRequest: (nextFilter: { urls: string[] }, nextListener: typeof listener) => {
      filter = nextFilter; listener = nextListener;
    } } };
    installNetworkBoundary(fakeSession as unknown as Parameters<typeof installNetworkBoundary>[0], { resolve: async () => ['127.0.0.1'] });
    expect(filter?.urls).toEqual(expect.arrayContaining(['http://*/*', 'https://*/*', 'ws://*/*', 'wss://*/*']));
    const result = await new Promise<{ cancel: boolean }>(resolve => listener!({ url: 'https://cdn.example/private.js' }, resolve));
    expect(result).toEqual({ cancel: true });
    await expect(validateNetworkRequestUrl('wss://socket.example/events', { resolve: async () => ['10.2.3.4'] }))
      .rejects.toSatisfy(expectCode('PRIVATE_NETWORK_BLOCKED'));
    await expect(validateNetworkRequestUrl('https://redirect-target.example/image.png', { resolve: async () => ['192.168.1.8'] }))
      .rejects.toSatisfy(expectCode('PRIVATE_NETWORK_BLOCKED'));
  });
});
...
describe('two-phase execution gate', () => {
  it('requires a grant and never calls verifier when absent', async () => {
    const f = await observed();
    await expect(f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref, effect: { kind: 'click' },
  ...
  it('fences an older prepared token when a newer one is prepared', async () => {
    ...
    const first = await f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref,
      effect: { kind: 'click' }, grant: 'grant-a',
    });
    const second = await f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref,
      effect: { kind: 'click' }, grant: 'grant-b',
    });

    await expect(f.service.executePrepared({ principalId: 'owner', executionToken: first.executionToken }))
      .rejects.toSatisfy(expectCode('STALE_FENCING_TOKEN'));
    await expect(f.service.executePrepared({ principalId: 'owner', executionToken: second.executionToken }))
  ...
  it('invalidates prepared tokens when a principal is revoked', async () => {
    ...
    const prepared = await f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref, effect: { kind: 'click' }, grant: 'grant',
    });
    f.service.invalidatePrincipal('owner');
    await expect(f.service.executePrepared({ principalId: 'owner', executionToken: prepared.executionToken }))
  ...
  it('cancels an in-flight effect when authorization is revoked', async () => {
    ...
    const execution = f.service.executePrepared({ principalId: 'owner', executionToken: prepared.executionToken });
    await dispatched;
    f.service.invalidatePrincipal('owner');
...
describe('select/check/press target gates', () => {
  it('validates select tag and observed option before consuming a grant', async () => {
    ...
    await expect(invalid.service.prepareEffect({
      principalId: 'owner', tabId: invalid.tabId, elementRef: invalid.ref,
      effect: { kind: 'select', value: 'prod' }, grant: 'grant',
    ...
    const prepared = await valid.service.prepareEffect({
      principalId: 'owner', tabId: valid.tabId, elementRef: valid.ref,
      effect: { kind: 'select', value: 'prod' }, grant: 'grant',
    });
    await expect(valid.service.executePrepared({ principalId: 'owner', executionToken: prepared.executionToken }))
      .resolves.toEqual({ ok: true });
    ...
    await expect(missingObserved.service.prepareEffect({
      principalId: 'owner', tabId: missingObserved.tabId, elementRef: missingObserved.ref,
      effect: { kind: 'select', value: 'prod' }, grant: 'grant',
  ...
  it('allows check only for checkbox/radio and carries desired state through the prepared effect', async () => {
    ...
    await expect(invalid.service.prepareEffect({
      principalId: 'owner', tabId: invalid.tabId, elementRef: invalid.ref,
      effect: { kind: 'check', checked: true }, grant: 'grant',
    })).rejects.toSatisfy(expectCode('UNSUPPORTED_ELEMENT'));
    ...
    await valid.service.executePrepared({ principalId: 'owner', executionToken: prepared.executionToken });
  ...
  it('binds an allowlisted press and controlled modifier into the one-shot execution', async () => {
    ...
    await expect(rejected.service.prepareEffect({
      principalId: 'owner', tabId: rejected.tabId, elementRef: rejected.ref,
      effect: { kind: 'press', key: 'F4', modifiers: ['Alt'] } as unknown as BrowserEffect, grant: 'grant',
    ...
    const prepared = await f.service.prepareEffect({
      principalId: 'owner', tabId: f.tabId, elementRef: f.ref, effect, grant: 'grant',
    });
    expect(f.verifier.verify).toHaveBeenCalledWith('grant', expect.objectContaining({ effect }));
    await f.service.executePrepared({ principalId: 'owner', executionToken: prepared.executionToken });
