...
describe('MVP integration security boundary', () => {
  ...
  // 没有历史可走时 goBack/goForward 什么也不做，这时若仍然挂上录制原因，它会一直留到
  ...
  it('labels a back or forward press only after confirming history can actually navigate', () => {
    ...
    const handlers = main.slice(
      main.indexOf('handle(IPC.tabBack'),
    ...
    expect(forwardGuard).toBeGreaterThanOrEqual(0);
    expect(backGuard).toBeLessThan(backCause);
    expect(forwardGuard).toBeLessThan(forwardCause);
  });
